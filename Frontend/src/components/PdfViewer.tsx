...
export const PdfViewer: React.FC<PdfViewerProps> = ({
  ...
  const renderSmartText = () => {
    if (isExtracting) {
      return (
        <div className="flex animate-pulse flex-col items-center justify-center p-20">
          ...
          <p className="text-muted-foreground font-medium">Extracting clean presentation text...</p>
        </div>
      )
    }
    ...
    if (!activeFindingEvent || !activeFindingEvent.finding.original_text) {
      return (
        <div className="mx-auto max-w-4xl p-12 text-foreground/90 font-serif text-lg leading-loose whitespace-pre-wrap">
          {extractedText}
        </div>
      )
    }
    ...
    if (startIndexInClean !== -1 && cleanSearch.length > 0) {
      ...
      const originalEndIndex = alphaNumMap[startIndexInClean + cleanSearch.length - 1];
      
      const before = extractedText.substring(0, originalStartIndex);
      const match = extractedText.substring(originalStartIndex, originalEndIndex + 1);
      const after = extractedText.substring(originalEndIndex + 1);
      
      return (
        <div className="mx-auto max-w-4xl p-12 text-foreground/90 font-serif text-lg leading-loose whitespace-pre-wrap tracking-wide">
          <span>{before}</span>
          <mark 
            id="active-highlight" 
            className="bg-destructive/20 text-destructive border-b-2 border-destructive px-1 py-0.5 rounded-sm font-bold shadow-sm"
          >
            {match}
          </mark>
          <span>{after}</span>
    ...
    return (
      <div className="mx-auto max-w-4xl p-12 text-foreground/90 font-serif text-lg leading-loose whitespace-pre-wrap relative">
        <div className="absolute top-4 right-4 bg-yellow-500/10 text-yellow-600 px-3 py-1 rounded-full text-xs font-semibold border border-yellow-500/20">
  ...
  return (
    <div className="flex h-full flex-col bg-muted/30">
      ...
      <div className="flex shrink-0 items-center justify-between border-b bg-background px-4 py-3 shadow-sm">
        <h2 className="truncate text-sm font-medium text-foreground max-w-[200px]">
          {file.name}
        </h2>
        ...
        <div className="flex items-center gap-1 bg-muted/50 p-1 rounded-lg">
          <button 
            ...
            className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all ${viewMode === "TEXT" ? "bg-background shadow-xs text-foreground" : "text-muted-foreground hover:text-foreground hover:bg-muted/80"}`}
          >
            Smart Text
          </button>
          <button 
            onClick={() => setViewMode("PDF")}
            className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all ${viewMode === "PDF" ? "bg-background shadow-xs text-foreground" : "text-muted-foreground hover:text-foreground hover:bg-muted/80"}`}
          >
            Original PDF
          </button>
        </div>
